...
static double clamp_frequency(double ppm) {
  if (ppm > FREQ_OFFSET_MAX_PPM)
    return FREQ_OFFSET_MAX_PPM;
  if (ppm < -FREQ_OFFSET_MAX_PPM)
    return -FREQ_OFFSET_MAX_PPM;
  return ppm;
}
...
int update_frequency_discipline(int64_t offset_us, int poll_exp) {
  ...
  if (g_freq_state.state == FREQ_STATE_NSET) {
    ...
    if (delta >= tc) {
      ...
      new_ppm =
          old_ppm + (new_ppm_calc - old_ppm) / (double)(1 << CLOCK_FLLGAIN);
      g_freq_state.ppm = new_ppm;
  ...
  if (recv_len > 0) {
    ...
    if (parse_ntp_packet(buffer, (size_t)recv_len, &pkt)) {
      ...
      if (handle_leap_indicator(li)) {
        ...
        if (mode != 4) {
          syslog(LOG_WARNING, "Не server mode пакет (mode=%u) от %s:%s", mode,
                 ip, port);
          close_socket(sock);
          return -1;
        }
        ...
        if (ntp_is_kod(&pkt)) {
          ...
          kod[4] = '\0';
          syslog(LOG_WARNING, "Kiss-o'-Death от %s:%s (code=%s)", ip, port,
                 kod);
          close_socket(sock);
          return -1;
        }
        ...
        if (pkt.xmit_ts.sec == 0 && pkt.xmit_ts.frac == 0) {
          syslog(LOG_WARNING, "Пустой transmit timestamp в ответе от %s:%s", ip,
                 port);
          close_socket(sock);
          return -1;
        }
        ...
        if (calculate_delay_offset(&t1, &pkt.recv_ts, &pkt.xmit_ts, &t4,
          ...
          if (abs_offset_us > MAXDIST) {
            syslog(LOG_WARNING,
                   "Пропуск коррекции: смещение слишком большое (%" PRId64
                   " мкс > %d мкс)",
                   offset_us, MAXDIST);
            pthread_mutex_lock(&g_mutex);
            g_local_stratum = 16;
          ...
          if (delay_us > MAXDIST * 10) {
            syslog(LOG_WARNING,
                   "Пропуск коррекции: задержка слишком большая (%" PRIu64
                   " мкс)",
                   delay_us);
            pthread_mutex_lock(&g_mutex);
            g_time_synced = false;
          ...
          g_local_poll = adjust_poll_interval(g_local_poll, g_peer_poll,
                                              delay_us, offset_us);

          marx_add_sample_us((uint64_t)ntp_timestamp_to_ns(&t4), delay_us,
                             offset_us);

          pthread_mutex_lock(&g_mutex);
          g_sample_count =
              marx_filter_outliers(g_samples, g_sample_count, MARX_K);
          ...
          g_local_poll =
              adjust_poll_interval(g_local_poll, pkt.poll, delay_us, offset_us);
          ...
          if (g_sample_count >= 1) {
            int apply_result =
                apply_time_correction_slew_or_step(filtered_offset);
            if (apply_result == 0) {
              syslog(LOG_DEBUG, "Коррекция применена: %" PRId64 " мкс",
                     filtered_offset);
            } else {
              syslog(LOG_WARNING, "Ошибка применения коррекции: %s",
                     strerror(errno));
            }
          ...
          uint64_t abs_off =
              (offset_us < 0) ? (uint64_t)(-offset_us) : (uint64_t)offset_us;
          uint64_t jitter_us = ntp_offset_jitter_us_locked();
          g_local_root_disp =
              update_root_dispersion(g_local_root_disp, abs_off, jitter_us);
          g_local_li = li;
          ...
          syslog(LOG_INFO,
                 "Синхронизация успешно завершена с %s:%s: задержка=%" PRIu64
                 " мкс, "

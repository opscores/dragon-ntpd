...
static int freq_file_write(double ppm) {
    ...
    if (fp == NULL) {
        syslog(LOG_DEBUG, "Cannot open frequency file for write: %s",
               strerror(errno));
        return -1;
    }
    fprintf(fp, "%.9f\n", ppm);
...
static double apply_loop_filter(double new_ppm, double* filter_ppm,
                                double alpha) {
    /* CERT C 3.4.5: Check for NULL pointer */
    if (filter_ppm == NULL) { return 0.0; }

    /* Apply deadband for small errors */
    if (fabs(*filter_ppm) < FREQ_DEADBAND_PPM &&
        fabs(new_ppm) < FREQ_DEADBAND_PPM) {
        return *filter_ppm;
    }
...
static int apply_freq_adjtime(double ppm) {
    ...
    if (ret < 0) {
        syslog(LOG_DEBUG, "adjtime frequency correction failed: %s",
               strerror(errno));
    }
    return ret;
}
...
static int apply_freq_adjtimex(double ppm) {
    ...
    int ret = adjtimex(&tx);
    if (ret < 0) {
        syslog(LOG_DEBUG, "adjtimex frequency correction failed: %s",
               strerror(errno));
        return -1;
    }
 ...
 * ============================================================================
 */
static double calculate_pll_ppm(int64_t offset_us, time_t delta_sec,
                                int poll_exp) {
    if (delta_sec <= 0) { return 0.0; }
    ...
    double dtemp =
        4.0 * pll_const * (double)poll_interval * (double)poll_interval;
...
static int update_frequency_discipline_internal(int64_t offset_us, int poll_exp,
                                                uint64_t jitter_us) {
    ...
    if (g_freq_state.state == FREQ_STATE_NSET) {
        ...
        if (delta >= tc) {
            ...
            new_ppm = old_ppm +
                      (new_ppm_calc - old_ppm) / (double)(1 << CLOCK_FLLGAIN);
            g_freq_state.ppm = new_ppm;
            g_freq_state.state = FREQ_STATE_SYNC;
        }
    } else {
        /* FREQ_STATE_SYNC - PLL mode with proper loop filter and gain
         * scheduling */
        time_t delta = now - g_freq_state.last_update;
        if (delta >= tc) {
            ...
            /* RFC 5905 Section 11.3: FLL for large poll intervals (tau >=
             * 2048s) */
            if (poll_exp >= 11) {
                ...
                if (avg_dt > 0) {
                    new_ppm_calc =
                        calculate_fll_ppm(offset_us, (time_t)avg_dt, poll_exp);
                } else {
                    new_ppm_calc = 0.0;
                }
            } else {
            ...
            double filtered_ppm = apply_loop_filter(
                new_ppm_calc, &g_freq_state.ppm_filter, PLL_ALPHA);
...
int update_frequency_discipline(int64_t offset_us, int poll_exp) {
    ...
    int result =
        update_frequency_discipline_internal(offset_us, poll_exp, jitter_us);
...
int apply_time_correction_slew_or_step(int64_t offset_us) {
    ...
    if (check_panic_condition(offset_us)) {
        syslog(LOG_WARNING,
               "Panic condition detected: time offset %lus exceeds threshold",
               offset_us / 1000);
        return -1;
    }
    ...
    if (offset_us > STEP_THRESHOLD_US || offset_us < -STEP_THRESHOLD_US) {
        ...
        int64_t ns =
            (int64_t)now_ts.tv_sec * 1000000000LL + (int64_t)now_ts.tv_nsec;

        // Check for overflow before addition
        if (offset_ns > 0 && ns > INT64_MAX - offset_ns) {
            syslog(LOG_ERR,
                   "Integer overflow in time correction: ns=%ld offset=%ld",
                   (long)ns, (long)offset_ns);
            return -1;
        }
        if (offset_ns < 0 && ns < INT64_MIN - offset_ns) {
            syslog(LOG_ERR,
                   "Integer underflow in time correction: ns=%ld offset=%ld",
                   (long)ns, (long)offset_ns);
            return -1;
        }
        ns += offset_ns;
...
uint32_t update_root_dispersion(uint32_t current_disp, uint64_t offset_us,
    ...
    double new_disp;
    if (phi_dispersion >
        (double)UINT32_MAX - (double)disp_us - (double)jitter_us_val) {
...
int sync_ntp_time(const char* ip, const char* port) {
    ...
    if (ret != 0 || res == NULL) {
        syslog(LOG_ERR, "Не удалось разрешить адрес: %s: %s", ip,
               gai_strerror(ret));
        return -1;
    }
    ...
    if (sendto(sock, request, sizeof(request), 0, (struct sockaddr*)&serv_addr,
               sizeof(serv_addr)) < 0) {
        syslog(LOG_ERR, "Ошибка отправки запроса на %s:%s: %s", ip, port,
               strerror(errno));
        close_socket(sock);
        return -1;
    }
    ...
    if (recv_len > 0) {
        /* RFC 5905 Section 11.2.1: Multi-server integration - process all peers
         * in pool */
        /* Update peer pool state with received packet */
        pthread_mutex_lock(&g_mutex);
        for (int i = 0; i < g_peer_pool_count; i++) {
            if (strncmp(g_peer_pool[i].ip, ip, sizeof(g_peer_pool[i].ip) - 1) ==
                    0 &&
                strncmp(g_peer_pool[i].port, port,
                        sizeof(g_peer_pool[i].port) - 1) == 0) {
        ...
        if (parse_ntp_packet(buffer, (size_t)recv_len, &pkt)) {
            uint8_t li =
                (uint8_t)((pkt.li_vn_mode & NTP_LI_MASK) >> NTP_LI_SHIFT);
            uint8_t mode = (uint8_t)(pkt.li_vn_mode & NTP_MODE_MASK);

            if (handle_leap_indicator(li)) {
                syslog(LOG_WARNING,
                       "Пропускаем обработку из-за Leap Indicator");
            } else {
                if (mode != 4) {
                    syslog(LOG_WARNING,
                           "Не server mode пакет (mode=%u) от %s:%s", mode, ip,
                           port);
                    close_socket(sock);
                    return -1;
                }
                ...
                if (ntp_is_kod(&pkt)) {
                    ...
                    kod[4] = '\0';
                    syslog(LOG_WARNING, "Kiss-o'-Death от %s:%s (code=%s)", ip,
                           port, kod);
                    close_socket(sock);
                    return -1;
                }
                ...
                if (pkt.orig_ts.sec != t1.sec || pkt.orig_ts.frac != t1.frac) {
                    syslog(
                        LOG_WARNING,
                        "Originate timestamp не совпал (spoof/late packet?)");
                    close_socket(sock);
                    return -1;
                }
                ...
                if (pkt.xmit_ts.sec == 0 && pkt.xmit_ts.frac == 0) {
                    syslog(LOG_WARNING,
                           "Пустой transmit timestamp в ответе от %s:%s", ip,
                           port);
                    close_socket(sock);
                    return -1;
                }
                ...
                uint64_t delay_us;
                int64_t offset_us;
                if (calculate_delay_offset(&t1, &pkt.recv_ts, &pkt.xmit_ts, &t4,
                                           &delay_us, &offset_us)) {
                    uint8_t peer_stratum = pkt.stratum;
                    uint8_t stratum = ntp_local_stratum_from_peer(peer_stratum);

                    /* RFC 5905 Section 11.3: Do not correct if peer is not
                     * reliable */
                    if (peer_stratum == 0 || peer_stratum > 15) {
                        syslog(
                            LOG_WARNING,
                            "Пропуск коррекции: ненадёжный сервер (stratum=%u)",
                            peer_stratum);
                        pthread_mutex_lock(&g_mutex);
                        g_local_stratum = 16;
                        g_time_synced = false;
                    ...
                    /* RFC 5905 Section 11.3: Do not step if offset exceeds
                     * MAXDIST (1 sec) */
                    int64_t abs_offset_us =
                        (offset_us >= 0) ? offset_us : -offset_us;
                    if (abs_offset_us > MAXDIST) {
                        syslog(LOG_WARNING,
                               "Пропуск коррекции: смещение слишком большое "
                               "(%" PRId64 " мкс > %d мкс)",
                               offset_us, MAXDIST);
                        pthread_mutex_lock(&g_mutex);
                        g_local_stratum = 16;
                    ...
                    /* RFC 5905 Section 11.3: Check for excessive delay (Bogus
                     * packet detection) */
                    if (delay_us > MAXDIST * 10) {
                        syslog(LOG_WARNING,
                               "Пропуск коррекции: задержка слишком большая "
                               "(%" PRIu64 " мкс)",
                               delay_us);
                        pthread_mutex_lock(&g_mutex);
                        g_time_synced = false;
                    ...
                    /* RFC 5905 Section 11.2.1: Calculate network quality for
                     * peer selection */
                    uint8_t network_quality =
                        calculate_network_quality(delay_us);
                    syslog(LOG_DEBUG,
                           "Network quality: %u%% (delay=%" PRIu64 " мкс)",
                           network_quality, delay_us);

                    /* RFC 5905 Section 11.2.1: Use compute_system_offset() for
                     * Byzantine fault detection */
                    /* For single peer: use offset directly, for pool: use
                     * majority vote
                     */
                    int best_idx = 0;
                    (void)compute_system_offset(
                        &offset_us, 1,
                        &best_idx); /* Suppress unused variable warning */
                    syslog(LOG_DEBUG, "System offset computed (best_idx=%d)",
                           best_idx);
                    ...
                    if (gettimeofday(&tv_now, NULL) == 0) {
                        uint64_t now_ns =
                            (uint64_t)tv_now.tv_sec * 1000000000ULL +
                    ...
                    int valid_count =
                        marx_filter_outliers(g_samples, g_sample_count, 3);
                    if (valid_count == 0) {
                        syslog(LOG_WARNING, "MARX: No valid samples after "
                                            "filtering, skipping correction");
                        pthread_mutex_lock(&g_mutex);
                        g_time_synced = false;
                        pthread_mutex_unlock(&g_mutex);
                        close_socket(sock);
                    ...
                    syslog(LOG_DEBUG, "Valid samples after MARX filtering: %d",
                           valid_count);

                    /* Update poll interval under mutex */
                    g_local_poll = adjust_poll_interval(g_local_poll, pkt.poll,
                                                        delay_us, offset_us);
                    ...
                    if (valid_count >= 1) {
                        int apply_result =
                            apply_time_correction_slew_or_step(offset_us);
                        if (apply_result == 0) {
                            syslog(LOG_DEBUG,
                                   "Коррекция применена: %" PRId64 " мкс",
                                   offset_us);
                        } else {
                            syslog(LOG_WARNING,
                                   "Ошибка применения коррекции: %s",
                                   strerror(errno));
                        }
                        ...
                        syslog(LOG_DEBUG,
                               "Пропуск коррекции: недостаточно выборок");
                    }
                    ...
                    uint64_t abs_off = (offset_us < 0) ? (uint64_t)(-offset_us)
                                                       : (uint64_t)offset_us;
                    uint64_t jitter_us = ntp_offset_jitter_us_locked();
                    g_local_root_disp = update_root_dispersion(
                        g_local_root_disp, abs_off, jitter_us);
                    g_local_li = li;
                    ...
                    syslog(LOG_INFO,
                           "Синхронизация успешно завершена с %s:%s: "
                           "задержка=%" PRIu64 " мкс, "
                           "страта=%u, смещение=%" PRId64 " мкс",
                           ip, port, delay_us, stratum, offset_us);

                    /* RFC 5905 Section 11.4: Check for leap second events after
                     * sync */
                    if (leap_second_integration_check() == 0) {
                        syslog(LOG_INFO,
                               "Leap second check completed after sync");
                    } else {
                        syslog(LOG_WARNING,
                               "Leap second check failed after sync: %s",
                               strerror(errno));
                    }
...
void update_clock_accuracy(int64_t offset_us, time_t delta_sec) {
    ...
    syslog(LOG_DEBUG, "Clock accuracy: precision=%.6f, stability=%.6f",
           state->precision, state->stability_metric);
}

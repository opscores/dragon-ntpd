...
enum { DEFAULT_NTP_PORT = 123, DEFAULT_NTPQ_PORT = 323, MIN_BUFFER_SIZE = 1024, MAX_BUFFER_SIZE = (4 * 1024 * 1024) };
...
int create_network_socket(const NetworkConfig* config, char* error_buf, size_t error_buf_size) {
    ...
    if (!config) {
        if (error_buf && error_buf_size > 0) { snprintf(error_buf, error_buf_size, "create_network_socket: config is NULL"); }
        return -1;
    }

    switch (config->family_preference) {
    case NETWORK_FAMILY_IPV4_ONLY: sock = network4_create_socket(config->ntp_port); break;
    case NETWORK_FAMILY_IPV6_ONLY: sock = network6_create_socket(config->ntp_port); break;
    case NETWORK_FAMILY_DUAL_STACK:
    default: sock = network6_create_socket(config->ntp_port); break;
    }

    if (sock < 0) {
        if (error_buf && error_buf_size > 0) { snprintf(error_buf, error_buf_size, "create_network_socket: socket() failed: %s", get_error_msg(errno)); }
        return -1;
    }
    ...
    if (config->family_preference == NETWORK_FAMILY_IPV6_ONLY) { network6_set_v6only(sock, true); }
        ...
        if (send_sz > 0 && recv_sz > 0) {
            ...
            if (ret < 0) {
                if (error_buf && error_buf_size > 0) {
                    snprintf(error_buf, error_buf_size, "create_network_socket: set_network_buffers failed: %s", get_error_msg(errno));
                }
                close(sock);
                return -1;
            }
    ...
    if (config->enable_multicast && config->multicast.enabled) {
        ...
        if (ret < 0) {
            if (error_buf && error_buf_size > 0) {
                snprintf(error_buf, error_buf_size, "create_network_socket: network_init_multicast failed: %s", get_error_msg(errno));
            }
            close(sock);
            return -1;
        }
...
int accept_network_connection(int sock, NetworkClientInfo* client_info) {
    ...
    if (sock_type == SOCK_STREAM) {
        ...
        ret = recvfrom(sock, NULL, 0, MSG_PEEK, (struct sockaddr*)&client_addr, &addr_len);
        if (ret < 0) return -1;
    }
    ...
    if (client_addr.ss_family == AF_INET) {
        ...
        if (inet_ntop(AF_INET, &addr4->sin_addr, client_info->address_str, sizeof(client_info->address_str)) == NULL) { client_info->address_str[0] = '\0'; }
        snprintf(client_info->address_family_str, sizeof(client_info->address_family_str), "IPv4");
        ...
        if (inet_ntop(AF_INET6, &addr6->sin6_addr, client_info->address_str, sizeof(client_info->address_str)) == NULL) { client_info->address_str[0] = '\0'; }
        snprintf(client_info->address_family_str, sizeof(client_info->address_family_str), "IPv6");
    } else {
        errno = EAFNOSUPPORT;
        return -1;
    }
...
ssize_t sendto_network(int sock, const void* buffer, size_t length, const NetworkAddress* dest) {
    ...
    if (dest->family == AF_INET) {
        ...
        if (dest->addr.addr_in6.sin6_scope_id > 0) { addr6.sin6_scope_id = dest->addr.addr_in6.sin6_scope_id; }
...
ssize_t recvfrom_network(int sock, void* buffer, size_t length, NetworkClientInfo* client_info, int flags) {
    ...
    if (client_info) memset(client_info, 0, sizeof(*client_info));

    memset(&client_addr, 0, sizeof(client_addr));
    addr_len = sizeof(client_addr);

    received = recvfrom(sock, buffer, length, flags, (struct sockaddr*)&client_addr, &addr_len);

    if (received < 0) return -1;

    if (addr_len == 0) return received;

    if (!client_info) return received;
 ...
 * @return 0 on success, -1 on error
 */
int parse_network_address(const char* addr_str, in_port_t port, int family, NetworkAddress* addr) {
...
size_t format_network_address(const NetworkAddress* addr, char* buf, size_t buf_size) {
    ...
    if (addr->family == AF_INET) {
        if (inet_ntop(AF_INET, &addr->addr.addr_in4.sin_addr, addr_str, sizeof(addr_str)) == NULL) {
            buf[0] = '\0';
            return 0;
        }
        snprintf(buf, buf_size, "%s:%u", addr_str, nport(addr->port));
    } else if (addr->family == AF_INET6) {
        if (inet_ntop(AF_INET6, &addr->addr.addr_in6.sin6_addr, addr_str, sizeof(addr_str)) == NULL) {
            buf[0] = '\0';
            return 0;
        }
        snprintf(buf, buf_size, "[%s]:%u", addr_str, nport(addr->port));
...
bool is_loopback_address(const NetworkAddress* addr) {
    ...
    if (addr->family == AF_INET) {
        return (addr->addr.addr_in4.sin_addr.s_addr == INADDR_LOOPBACK);
    } else if (addr->family == AF_INET6) {
...
bool addresses_equal(const NetworkAddress* addr1, const NetworkAddress* addr2) {
    ...
    if (addr1->family == AF_INET) {
        return (addr1->addr.addr_in4.sin_addr.s_addr == addr2->addr.addr_in4.sin_addr.s_addr);
    } else if (addr1->family == AF_INET6) {
        return IN6_ARE_ADDR_EQUAL(&addr1->addr.addr_in6.sin6_addr, &addr2->addr.addr_in6.sin6_addr);
    }
 ...
 * @return 0 on success, -1 on error
 */
int join_multicast_group(int sock, const char* group, in_port_t port, int ttl, int interface_index) {
    ...
    mreq.ipv6mr_interface = (interface_index > 0) ? (uint32_t)interface_index : 0;
    ...
    if (ttl > 0) {
        int ttl_val = ttl;
        int ttl_ret = setsockopt(sock, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, &ttl_val, sizeof(ttl_val));
        if (ttl_ret < 0) return -1;
    }
...
int leave_multicast_group(int sock, const char* group, in_port_t port) {
    ...
    ret = setsockopt(sock, IPPROTO_IPV6, IPV6_LEAVE_GROUP, &mreq, sizeof(mreq));
    if (ret < 0) return -1;
...
int network_init_multicast(int sock, const NetworkConfig* config) {
    ...
    if (!config->multicast.enabled) return 0;

    ret = join_multicast_group(sock, config->multicast.address, config->multicast.port, config->multicast.ttl, config->multicast.interface_index);
    if (ret < 0) return -1;
...
int network_cleanup_multicast(int sock) {
    if (!is_valid_socket(sock)) return 0;
...
int set_network_buffers(int sock, int send_size, int recv_size) {
    ...
    if (ssz > 0) {
        ...
        if (setsockopt(sock, SOL_SOCKET, SO_SNDBUF, &ssz, sizeof(ssz)) < 0) return -1;
    }
    ...
    if (rsz > 0) {
        ...
        if (setsockopt(sock, SOL_SOCKET, SO_RCVBUF, &rsz, sizeof(rsz)) < 0) return -1;
    }
...
int enable_ipv6_v6only(int sock, bool v6only) {
    ...
    return setsockopt(sock, IPPROTO_IPV6, IPV6_V6ONLY, &v6only_val, sizeof(v6only_val));
}
...
int network_config_validate(const NetworkConfig* config) {
    ...
    if (config->ntp_port == 0) return -1;

    if (config->family_preference > NETWORK_FAMILY_DUAL_STACK) return -1;

    if (config->interface[0] != '\0') {
        size_t len = strnlen(config->interface, sizeof(config->interface));
        if (len >= NETWORK_MAX_IFNAME_LEN) return -1;
    }

    if (config->multicast.enabled) {
        if (config->multicast.address[0] == '\0') return -1;
    }

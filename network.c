 ...
 */
static inline int is_valid_socket(int sock) { return (sock >= 0) ? 1 : 0; }
 ...
 */
static inline in_port_t hport(in_port_t port) { return htons(port); }
 ...
 */
static inline in_port_t nport(in_port_t port) { return ntohs(port); }
...
int accept_network_connection(int sock, NetworkClientInfo *client_info) {
  ...
  if (sock_type == SOCK_STREAM) {
    ...
    ret = recvfrom(sock, NULL, 0, MSG_PEEK, (struct sockaddr *)&client_addr,
                   &addr_len);
    if (ret < 0)
      return -1;
  }
  ...
  if (client_addr.ss_family == AF_INET) {
    ...
    if (inet_ntop(AF_INET, &addr4->sin_addr, client_info->address_str,
                  sizeof(client_info->address_str)) == NULL) {
      client_info->address_str[0] = '\0';
    }
    snprintf(client_info->address_family_str,
             sizeof(client_info->address_family_str), "IPv4");
    ...
    if (inet_ntop(AF_INET6, &addr6->sin6_addr, client_info->address_str,
                  sizeof(client_info->address_str)) == NULL) {
      client_info->address_str[0] = '\0';
    }
    snprintf(client_info->address_family_str,
             sizeof(client_info->address_family_str), "IPv6");
  } else {
    errno = EAFNOSUPPORT;
    return -1;
  }
 ...
 * @return 0 on success, -1 on error
 */
int parse_network_address(const char *addr_str, in_port_t port, int family,
                          NetworkAddress *addr) {
...
size_t format_network_address(const NetworkAddress *addr, char *buf,
  ...
  if (addr->family == AF_INET) {
    if (inet_ntop(AF_INET, &addr->addr.addr_in4.sin_addr, addr_str,
    ...
    if (inet_ntop(AF_INET6, &addr->addr.addr_in6.sin6_addr, addr_str,
                  sizeof(addr_str)) == NULL) {
      buf[0] = '\0';
      return 0;
    }
 ...
 * @return 0 on success, -1 on error
 */
int join_multicast_group(int sock, const char *group, in_port_t port, int ttl,
                         int interface_index) {
  ...
  if (ttl > 0) {
    int ttl_val = ttl;
    int ttl_ret = setsockopt(sock, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, &ttl_val,
                             sizeof(ttl_val));
    if (ttl_ret < 0)
      return -1;
  }
...
int network_init_multicast(int sock, const NetworkConfig *config) {
  ...
  ret = join_multicast_group(sock, config->multicast.address,
                             config->multicast.port, config->multicast.ttl,
...
int set_network_buffers(int sock, int send_size, int recv_size) {
  ...
  if (ssz > 0) {
    ...
    if (setsockopt(sock, SOL_SOCKET, SO_SNDBUF, &ssz, sizeof(ssz)) < 0)
      return -1;
  }
  ...
  if (rsz > 0) {
    ...
    if (setsockopt(sock, SOL_SOCKET, SO_RCVBUF, &rsz, sizeof(rsz)) < 0)
      return -1;
  }
...
int enable_ipv6_v6only(int sock, bool v6only) {
  ...
  return setsockopt(sock, IPPROTO_IPV6, IPV6_V6ONLY, &v6only_val,
                    sizeof(v6only_val));
}

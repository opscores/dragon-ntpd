                        ...
                        const struct sockaddr_in *dest)
{
    ...
    if (sent < 0) {
        syslog(LOG_DEBUG, "IPv4: sendto failed: %s", strerror(errno));
    }
                          ...
                          struct sockaddr_in *src_addr)
{
    ...
    ssize_t received = recvfrom(sock, buf, len, 0,
                                (struct sockaddr *)src_addr, &addr_len);

    if (received < 0) {
        if (errno != EAGAIN && errno != EWOULDBLOCK) {
            syslog(LOG_DEBUG, "IPv4: recvfrom failed: %s", strerror(errno));
        }
        return -1;
    }
...
size_t network4_format_address(const struct sockaddr_in *addr,

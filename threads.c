...
typedef struct {
  ...
  atomic_bool
      sock_valid; /* Флаг валидности сокетa (atomic для signal safety) */
...
static int handle_peer_request(const void *buffer, size_t size, const char *ip,
  ...
  if (g_peer_ctx.sock_valid) {
    ...
    ssize_t send_len =
        sendto(g_peer_ctx.sock_fd, &response, sizeof(response), 0,
...
static void *peer_thread_main(void *arg) {
  ...
  syslog(LOG_INFO, "Поток обработки пэера запущен для %s:%s", ctx->ip,
         ctx->port);
  ...
  while (atomic_load_explicit(&g_peer_thread_running, memory_order_acquire)) {
    ...
    if (inet_ntop(
            AF_INET,
            &((struct sockaddr_in *)&g_peer_ctx.client_addr_storage)->sin_addr,
            client_ip, sizeof(client_ip)) == NULL) {
      snprintf(client_ip, sizeof(client_ip), "%s", "unknown");
    }
    char port_str[6];
    snprintf(
        port_str, sizeof(port_str), "%hu",
...
void stop_peer_thread(void) {
  ...
  if (join_ret != 0) {
    syslog(LOG_WARNING, "Ошибка pthread_join потока пэера: %s",
           strerror(join_ret));
  }
...
void cleanup_peer_thread(void) {
  ...
  if (pthread_mutex_destroy(&g_peer_ctx.sock_mutex) != 0) {
    syslog(LOG_WARNING, "Ошибка pthread_mutex_destroy пэера: %s",
           strerror(errno));
  }
  if (pthread_cond_destroy(&g_peer_ctx.sock_cond) != 0) {
    syslog(LOG_WARNING, "Ошибка pthread_cond_destroy пэера: %s",
           strerror(errno));
  }
}
...
static void *clock_thread_main(void *arg) {
  ...
  while (atomic_load_explicit(&g_clock_thread_running, memory_order_acquire)) {
    ...
    int64_t expected_ns =
        (int64_t)g_last_sync_ts.sec * 1000000000LL + g_last_sync_ts.frac * 1000;

...
int parse_arguments(int argc, char *argv[]) {
  ...
  while (i < argc) {
    ...
    if (strcmp(arg, "-c") == 0 || strcmp(arg, "--config") == 0) {
      ...
      if (parse_positive_int(arg + 3, 0, MAX_DEBUG_LEVEL, &g_cli.debug_level) !=
          0) {
        fprintf(stderr, "Error: -D= requires 0-%d\n", MAX_DEBUG_LEVEL);
        return -1;
      }
    } else if (strncmp(arg, "--debug=", 8) == 0) {
      if (parse_positive_int(arg + 8, 0, MAX_DEBUG_LEVEL, &g_cli.debug_level) !=
          0) {
      ...
      if (i + 1 < argc && argv[i + 1][0] != '-') {
        if (parse_positive_int(argv[i + 1], MIN_TIMEOUT_SEC, MAX_TIMEOUT_SEC,
                               &g_cli.timeout_sec) != 0) {

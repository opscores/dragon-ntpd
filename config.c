...
void print_usage(const char* prog) {
    printf("DNTPD v%s - Dragon NTP Server (RFC 5905/5906/8915 compliant)\n\n",
           VERSION);
    ...
    printf(
        "  -t, --timeout=SEC  Sync timeout in seconds (%d-%d, default: %d)\n",
...
static int parse_positive_int(const char* str, int min_val, int max_val,
                              int* out) {
    char* endptr = NULL;
    long val;
    ...
    while (i < argc) {
        ...
        if (strcmp(arg, "-c") == 0 || strcmp(arg, "--config") == 0) {
            ...
            g_cli.foreground = 1;
                   ...
                   strcmp(arg, "--no-daemonize") == 0) {
            if (g_cli.foreground)
                fprintf(stderr, "Note: -f has no effect (same as -n)\n");
            g_cli.no_daemonize = 1;
        } else if (strcmp(arg, "-D") == 0) {
            if (i + 1 < argc && argv[i + 1][0] != '-') {
                if (parse_positive_int(argv[i + 1], 0, MAX_DEBUG_LEVEL,
                                       &g_cli.debug_level) != 0) {
            ...
            if (parse_positive_int(arg + 3, 0, MAX_DEBUG_LEVEL,
                                   &g_cli.debug_level) != 0) {
                fprintf(stderr, "Error: -D= requires 0-%d\n", MAX_DEBUG_LEVEL);
                return -1;
            }
        } else if (strncmp(arg, "--debug=", 8) == 0) {
            if (parse_positive_int(arg + 8, 0, MAX_DEBUG_LEVEL,
                                   &g_cli.debug_level) != 0) {
            ...
            if (i + 1 < argc && argv[i + 1][0] != '-') {
                if (parse_positive_int(argv[i + 1], MIN_TIMEOUT_SEC,
                                       MAX_TIMEOUT_SEC,
                                       &g_cli.timeout_sec) != 0) {
            ...
            if (parse_positive_int(arg + 10, MIN_TIMEOUT_SEC, MAX_TIMEOUT_SEC,
                                   &g_cli.timeout_sec) != 0) {

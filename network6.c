...
int network6_create_socket(uint16_t port) {
    ...
    int reuse = 1;
    if (setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, &reuse, sizeof(reuse)) < 0) { syslog(LOG_WARNING, "IPv6: SO_REUSEADDR failed: %s", strerror(errno)); }

    int v6only = 0;
    if (setsockopt(sock, IPPROTO_IPV6, IPV6_V6ONLY, &v6only, sizeof(v6only)) < 0) { syslog(LOG_WARNING, "IPv6: IPV6_V6ONLY failed: %s", strerror(errno)); }
...
int network6_bind_socket(int sock, const char* interface, uint16_t port) {
    ...
    if (interface != NULL && interface[0] != '\0') {
        ...
        if (ifindex > 0) {
            ...
            syslog(LOG_INFO, "IPv6: binding to interface %s (index=%u)", interface, ifindex);
        } else {
...
ssize_t network6_sendto(int sock, const void* buf, size_t len, const struct sockaddr_in6* dest) {
    ...
    if (sent < 0) { syslog(LOG_DEBUG, "IPv6: sendto failed: %s", strerror(errno)); }
...
ssize_t network6_recvfrom(int sock, void* buf, size_t len, struct sockaddr_in6* src_addr) {
    ...
    if (received < 0) {
        if (errno != EAGAIN && errno != EWOULDBLOCK) { syslog(LOG_DEBUG, "IPv6: recvfrom failed: %s", strerror(errno)); }
        return -1;
    }
...
int network6_parse_address(const char* addr_str, uint16_t port, struct sockaddr_in6* addr) {
    ...
    if (inet_pton(AF_INET6, addr_str, &addr->sin6_addr) <= 0) { return -1; }
...
int network6_join_multicast(int sock, const char* group_addr, int interface_index) {
    if (sock < 0 || group_addr == NULL) { return -1; }
    ...
    mreq.ipv6mr_interface = (interface_index > 0) ? (unsigned int)interface_index : 0;
...
int network6_leave_multicast(int sock, const char* group_addr, int interface_index) {
    ...
    if (inet_pton(AF_INET6, group_addr, &mreq.ipv6mr_multiaddr) <= 0) { return -1; }

...
bool ido_process_offer(IdoState* ido_state, uint16_t ef_type, uint8_t ef_length, const uint8_t* ef_data) {
    ...
    if (ef_type != IDO_EF_TYPE_OFFER) { return false; }
...
bool ido_process_response(IdoState* ido_state, uint16_t ef_type, uint8_t ef_length, const uint8_t* ef_data) {
    ...
    /* Validate extension field type */
    if (ef_type != IDO_EF_TYPE_RESPONSE) { return false; }
...
bool ido_process(IdoState* ido_state, uint16_t ef_type, uint8_t ef_length, const uint8_t* ef_data) {
    ...
    if (ef_type == IDO_EF_TYPE_OFFER) { return ido_process_offer(ido_state, ef_type, ef_length, ef_data); }

    /* Check if it's a Response (client -> server) */
    if (ef_type == IDO_EF_TYPE_RESPONSE) { return ido_process_response(ido_state, ef_type, ef_length, ef_data); }
...
bool ido_process_skip(IdoState* ido_state, uint16_t ef_type, uint8_t ef_length) {
    /* Validate extension field type */
    if (ef_type != IDO_EF_TYPE_OFFER && ef_type != IDO_EF_TYPE_RESPONSE) { return false; }
 ...
 *   │                          │                          │
 *   │                          │                          ├─ Authentication established ──→ AUTHENTICATED
 *   │                          │                          │
...
uint8_t ido_state_machine(IdoState* ido_state, uint8_t event) {
    ...
    if (new_state != ido_state->ido_state) {
        ...
        syslog(LOG_INFO, "I-DO: State transition: %s → %s", ido_state_name(prev_state), ido_state_name(new_state));
    }
...
void ido_log_state(const IdoState* ido_state) {
    ...
    syslog(LOG_INFO,
           ...
           ido_state->ido_capabilities, ido_state->ido_authenticated ? "yes" : "no");
}

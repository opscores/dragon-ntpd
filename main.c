...
int load_server_config(void) {
  ...
  while (fgets(line, sizeof(line), fp)) {
    ...
    ServerConfig *temp =
        realloc(g_servers, (size_t)(g_server_count + 1) * sizeof(ServerConfig));
    if (!temp) {
...
int main(int argc, char *argv[]) {
  ...
  if (g_cli.log_file != NULL) {
    ...
    if (log_fp != NULL) {
      ...
      syslog(LOG_WARNING, "Не удалось открыть log файл: %s: %s", g_cli.log_file,
             strerror(errno));
    }
  }

  g_local_precision = get_system_precision();
  syslog(LOG_INFO, "System precision: %d (2^%d = %.3f сек)", g_local_precision,
         g_local_precision,
         g_local_precision >= 0
  ...
  if (!g_cli.foreground && !g_cli.no_daemonize) {
    ...
    if (g_cli.pid_file != NULL) {
      ...
      if (pid_fp != NULL) {
        fprintf(pid_fp, "%d\n", (int)getpid());
        fclose(pid_fp);
        syslog(LOG_INFO, "PID файл записан: %s (PID=%d)", g_cli.pid_file,
               (int)getpid());
      } else {
